Feature: Testing the services in user-service
  ...
  Scenario: Adding a new client
    When created a new client with first name "Luka", second name "Basrak", email "test@example.com", adress "Sample Adress 3", phone number "1234567", gender "M", and birthday on "14.07.2001." and jmbg "1111111111111" and username "Luka"
    And list all clients
    Then recieve client with email "test@example.com"

  Scenario: Updating a client
    Given created a new client with first name "Luka", second name "Basrak", email "test1@example.com", adress "Sample Adress 3", phone number "1234567", gender "M", and birthday on "14.07.2001." and jmbg "1111111111112" and username "Luka1"
  ...
  Scenario: Deleting a client
    Given created a new client with first name "Luka", second name "Basrak", email "test2@example.com", adress "Sample Adress 3", phone number "1234567", gender "M", and birthday on "14.07.2001." and jmbg "1111111111113" and username "Luka2"
