...
class ChurnPrediction:
    ...
    def get_churn_indicators(self, client_id: int):
        ...
        query = select(
            ...
            func.count(case((and_(Payment.date >= ninety_days_ago, Payment.date < thirty_days_ago), Payment.id))).label('previous_transactions'),
            func.count(Card.id).label('card_count'),
            func.count(case((Card.type == CardType.CREDIT, Card.id))).label('credit_cards'),
            func.count(Loan.id).label('total_loans'),
            func.count(case((Loan.status == LoanStatus.APPROVED, Loan.id))).label('active_loans'),
            func.count(case((and_(Payment.date >= one_year_ago, Payment.status == PaymentStatus.COMPLETED), Payment.id))).label('yearly_successful_transactions'),
            func.count(case((Account.type == AccountType.FOREIGN, Account.account_number))).label('foreign_accounts'),
            func.count(case((Loan.status == LoanStatus.DELINQUENT, Loan.id))).label('delinquent_loans'),
            func.count(case((Installment.installment_status == InstallmentStatus.LATE, Installment.id))).label('late_installments'),
        ...
        days_since_creation = (current_date - result.creation_date.date() if isinstance(result.creation_date, datetime) else current_date - result.creation_date).days if result.creation_date else 0
